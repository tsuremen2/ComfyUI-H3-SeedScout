...
---------------------------------------------------------------------------
VENDORED, UNMODIFIED, from https://github.com/madebyollin/taehv @ 62f7591
(MIT License, Copyright (c) Ollin Boer Bohan).

Kept byte-identical to upstream apart from this header so it stays trivially
diffable against the pinned commit. ComfyUI-H3-SeedScout uses only the decoder
half (see `_TaehvDecoder` in nodes.py); the encoder is retained because the
published checkpoints carry encoder weights and TAEHV.__init__ builds both.

Requires only torch + tqdm, both of which ship with ComfyUI. The __main__
demo additionally imports cv2, but only inside main() — never at import time.
---------------------------------------------------------------------------
"""
import torch
